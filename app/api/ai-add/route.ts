...
export async function POST(req: NextRequest) {
  ...
  const prompt =
    ...
    '規則:\n' +
    '・文章は音声認識の文字起こしの場合があり、同音異字の誤変換を含みうる。読み（発音）が同じ・近い上場企業名に解釈して正式社名に直す（例:「早実」「デンソー掃除」の掃除→読み「そうじ(つ)」→「双日」、「トヨタ通商」→「豊田通商」、「三菱中古」→読み「みつびしじゅうこ(う)」→「三菱重工業」、「総合庁舎」→「総合商社」、「NVディア」→「NVIDIA」、「アーム」→「Arm Holdings」）。\n' +
    '・日本の通称は正式社名に直す（例:「トヨタ」→「トヨタ自動車」）。社名変更した企業は現在の社名を使う（例: 日本電産→ニデック、昭和電工→レゾナック）。\n' +
