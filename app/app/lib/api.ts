...
export async function fetchFinancialOne(apiKey: string, code: string): Promise<FinResult | null> {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      if (attempt > 0) {
        const wait = attempt <= 2 ? 800 * Math.pow(2, attempt) : 5000
      ...
      const wait429 = 2000 * Math.pow(2, attempt)
      if (is429) await new Promise(r => setTimeout(r, wait429))
      // 最終試行なら諦めてnullを返す（上位でリトライ済み）
    }
  }
  return null
}
