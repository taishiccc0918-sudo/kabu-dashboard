...
function DashboardTable({
  ...
  const cols: { label: string; cls: string; key: keyof StockRow | null; group: string; width?: number; tooltip?: string }[] = [
    ...
    { label: 'PER今期\n3ヶ月前比', cls: `${styles.thRight} ${styles.thPerGroup}`, key: 'perFChg3m' as keyof StockRow, group: 'per', tooltip: '過去FEPS基準の真のPER変化率（3ヶ月前）。\n(現在PER÷3M前PER−1)。必ず決算をまたぐため業績修正の影響が分離可能。\nセルにホバーで詳細（過去FEPS・現在FEPSも表示）' },
    { label: 'PER今期\n1年前比',   cls: `${styles.thRight} ${styles.thPerGroup}`, key: 'perFChg1y' as keyof StockRow, group: 'per', tooltip: '過去FEPS基準の真のPER変化率（1年前）。\n(現在PER÷1Y前PER−1)。4決算期をまたぎ最も信頼性が高い。\nセルにホバーで詳細（過去FEPS・現在FEPSも表示）' },
  ...
  const colWidths = [48,60,150,160,72,108,80,80,76,80,76,76,76,76,76,76,76,64,64,88,92,64,92,108,64,72,64,84,72,80]
  const colGroup = (
    <colgroup>
    ...
    <div className={styles.dashWrap}>
      ...
      <div className={styles.tbodyOuter} ref={bodyRef} onScroll={onBodyScroll}>
        <table className={styles.table}>
          ...
          <tbody>
            {filteredRows.length === 0 ? (
              <tr><td colSpan={30} className={styles.emptyCell}>該当銘柄なし</td></tr>
  ...
  return (
    <tr data-code={r.code} className={highlighted ? styles.trHighlight : undefined} style={{ cursor: 'pointer' }} onClick={onClick}>
      ...
      <td className={`${styles.tdPct} ${styles.tdPerGroup} ${fin?.feps === null ? styles.tdNonDisclosure : styles.hasTooltip}`}
        ...
        title={fin?.feps === null ? '業績予想を開示していない銘柄です' : (r.perFChg3mPrev && r.perF && fin?.feps3m) ? `3M前: PER ${fmtN(r.perFChg3mPrev)}倍 (FEPS ${fmtN(fin.feps3m, 0)}円) → 現在: PER ${fmtN(r.perF)}倍 (FEPS ${fmtN(fin.feps ?? null, 0)}円) ／ PER変化: ${fmtPct(r.perFChg3m)}` : undefined}
      ...
      <td className={`${styles.tdPct} ${styles.tdPerGroup} ${fin?.feps === null ? styles.tdNonDisclosure : styles.hasTooltip}`}
        ...
        title={fin?.feps === null ? '業績予想を開示していない銘柄です' : (r.perFChg1yPrev && r.perF && fin?.feps1y) ? `1Y前: PER ${fmtN(r.perFChg1yPrev)}倍 (FEPS ${fmtN(fin.feps1y, 0)}円) → 現在: PER ${fmtN(r.perF)}倍 (FEPS ${fmtN(fin.feps ?? null, 0)}円) ／ PER変化: ${fmtPct(r.perFChg1y)}` : undefined}
